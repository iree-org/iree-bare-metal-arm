...
// An example to demonstrate how to run a quantized MobileNet V1 model with
// IREE on bare-metal Arm.
//
...
iree_status_t Run() {
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_buffer_view_allocate_buffer(
        ...
        iree_make_const_byte_span(image, sizeof(uint8_t) * kElementCount),
        &arg0_buffer_view);
  }
