...
iree_status_t Run() {
  ...
  IREE_RETURN_IF_ERROR(iree_hal_allocator_allocate_buffer(
      ...
      IREE_HAL_BUFFER_USAGE_ALL, sizeof(int32_t) * kElementCount,
      &arg0_buffer));
  IREE_RETURN_IF_ERROR(iree_hal_allocator_allocate_buffer(
      iree_hal_device_allocator(device), input_memory_type,
      IREE_HAL_BUFFER_USAGE_ALL, sizeof(int32_t) * kElementCount,
  ...
  IREE_RETURN_IF_ERROR(iree_hal_buffer_view_create(
      arg0_buffer, shape, IREE_ARRAYSIZE(shape), IREE_HAL_ELEMENT_TYPE_SINT_32,
      IREE_HAL_ENCODING_TYPE_DENSE_ROW_MAJOR, &arg0_buffer_view));
  IREE_RETURN_IF_ERROR(iree_hal_buffer_view_create(
      arg1_buffer, shape, IREE_ARRAYSIZE(shape), IREE_HAL_ELEMENT_TYPE_SINT_32,
  ...
  for (int i = 0; i < mapped_memory.contents.data_length / sizeof(int32_t);
       ++i) {
    if (((const int32_t*)mapped_memory.contents.data)[i] != 8) {
